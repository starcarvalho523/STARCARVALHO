-- Prepare a tightly scoped Efí Pix Production canary without enabling the
-- global customer capability. No canary row is inserted by this migration.

create table if not exists private.efi_pix_production_canary_sessions (
...
create index if not exists efi_pix_production_canary_sessions_actor_idx
  on private.efi_pix_production_canary_sessions(actor_id);

revoke all on table private.efi_pix_production_canary_sessions
  from public, anon, authenticated;

create or replace function private.is_efi_pix_production_canary(
  target_session uuid,
  target_actor uuid
)
returns boolean
...
as $$
  select exists (
    select 1
    from private.efi_pix_production_canary_sessions c
...
revoke all on function private.is_efi_pix_production_canary(uuid,uuid)
  from public, anon, authenticated;

create or replace function public.is_efi_pix_production_canary_for_actor(
  target_session uuid,
  target_actor uuid
)
returns boolean
...
set search_path = pg_catalog, public, private
as $$
  select private.is_efi_pix_production_canary(target_session,target_actor);
$$;

revoke all on function public.is_efi_pix_production_canary_for_actor(uuid,uuid)
  from public, anon, authenticated;
grant execute on function public.is_efi_pix_production_canary_for_actor(uuid,uuid)
  to service_role;

create or replace function private.authorize_efi_pix_session_for_actor_environment(
  target_session uuid,
  target_actor uuid,
...
begin
  ...
  if target_environment = 'PRODUCTION' then
    production_canary := private.is_efi_pix_production_canary(target_session,target_actor);
  end if;
...
revoke all on function private.authorize_efi_pix_session_for_actor_environment(uuid,uuid,text)
  from public, anon, authenticated;

create or replace function public.get_or_reserve_efi_pix_payment_for_actor(
  target_session uuid,
  target_actor uuid,
  target_environment text
)
...
begin
  s := private.authorize_efi_pix_session_for_actor_environment(
    target_session,
    target_actor,
    target_environment
  );
...
revoke all on function public.get_or_reserve_efi_pix_payment_for_actor(uuid,uuid,text)
  from public, anon, authenticated;
grant execute on function public.get_or_reserve_efi_pix_payment_for_actor(uuid,uuid,text)
  to service_role;
