-- Enforce employee deactivation inside SECURITY DEFINER RPCs, not only in the UI.
-- Version matches the migration applied and retested on QA.
-- Current QA definitions retained; only the three actor authorization scopes change.
...
begin
  select exists(
    ...
    where uur.user_id=auth.uid()
      and exists(select 1 from public.profiles actor_profile where actor_profile.id=auth.uid() and actor_profile.is_active)
  ...
  with allowed_units as (
    ...
    where user_id=auth.uid() and exists(select 1 from public.profiles actor_profile where actor_profile.id=auth.uid() and actor_profile.is_active)
      and coalesce(is_active,true) and role::text in ('owner','manager')
  ),
  scoped_sessions as (
  ...
  scoped_vehicle_ids as (
    ...
    union
    select distinct s.vehicle_id from scoped_subs s where s.vehicle_id is not null
  ),
  scoped_payments as (
    select p.*
    ...
    where (p.parking_session_id in (select id from scoped_sessions))
       or (p.monthly_billing_period_id in (
          select bp.id from public.monthly_billing_periods bp
  ...
  select jsonb_build_object(
    'profile', jsonb_build_object(
      ...
      'full_name',coalesce(nullif(cp.full_name,''),nullif(pr.full_name,''),'Cliente'),
      'email',u.email,
      'is_active',coalesce(cp.is_active,pr.is_active,true),
      'created_at',coalesce(cp.created_at,u.created_at)
    ),
    'vehicles',coalesce((select jsonb_agg(jsonb_build_object(
      'id',v.id,'plate',v.plate,'vehicle_type',v.vehicle_type::text,'notes',v.notes,
      'last_visit_at',(select max(s.entered_at) from scoped_sessions s where s.vehicle_id=v.id),
      'has_active_session',exists(select 1 from scoped_sessions s where s.vehicle_id=v.id and s.status::text in ('OPEN','PAYMENT_PENDING','PAID'))
    ) order by v.plate) from public.vehicles v where v.customer_id=p_customer_id and v.id in(select id from scoped_vehicle_ids)),'[]'::jsonb),
    'sessions',coalesce((select jsonb_agg(jsonb_build_object(
      'id',s.id,'unit_id',s.unit_id,'unit_name',s.unit_name,'plate',s.plate_snapshot,'vehicle_type',s.vehicle_type::text,
      'status',s.status::text,'entered_at',s.entered_at,'exited_at',s.exited_at,'calculated_amount',s.calculated_amount,
      'final_amount',s.final_amount,'payment_status',s.payment_status::text,'entry_mode',s.entry_mode,'financial_obligation',s.financial_obligation
    ) order by s.entered_at desc) from scoped_sessions s),'[]'::jsonb),
    'payments',coalesce((select jsonb_agg(jsonb_build_object(
      'id',p.id,'unit_id',p.unit_id,'parking_session_id',p.parking_session_id,'amount',p.amount,'method',p.method::text,
      'status',p.status::text,'provider',p.provider,'paid_at',p.paid_at,'created_at',p.created_at,'payment_subject_type',p.payment_subject_type::text
    ) order by p.created_at desc) from scoped_payments p),'[]'::jsonb),
    'subscriptions',coalesce((select jsonb_agg(jsonb_build_object(
      'id',s.id,'unit_id',s.unit_id,'unit_name',s.unit_name,'plan_id',s.plan_id,'plan_name',s.plan_name,'status',s.status,
      'starts_on',s.starts_on,'ends_on',s.ends_on,'due_day',s.due_day,'grace_days',s.grace_days,'contracted_price',s.contracted_price,
      'vehicle_id',s.vehicle_id,'cancel_at_period_end',s.cancel_at_period_end
    ) order by s.created_at desc) from scoped_subs s),'[]'::jsonb),
    'billing_periods',coalesce((select jsonb_agg(jsonb_build_object(
      'id',b.id,'subscription_id',b.subscription_id,'reference_year',b.reference_year,'reference_month',b.reference_month,
      'due_date',b.due_date,'grace_until',b.grace_until,'amount',b.amount,'status',b.status,'paid_at',b.paid_at
    ) order by b.period_start desc) from scoped_billing b),'[]'::jsonb),
    'eligible_for_monthly',coalesce(cp.is_active,pr.is_active,true) and exists(
      select 1 from public.vehicles v join scoped_sessions s on s.vehicle_id=v.id where v.customer_id=p_customer_id
    )
  ) into result
  from auth.users u
...
revoke all on function public.get_ceo_customer_directory() from public, anon;
revoke all on function public.get_ceo_customer_detail(uuid) from public, anon;
