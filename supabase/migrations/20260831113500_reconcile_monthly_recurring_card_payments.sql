...
begin
  ...
  if current_count=1 then
    ...
    if payment_row.provider<>'ASAAS'
      ...
      update private.payment_provider_events set processing_status='REVIEW',payment_id=payment_row.id,processed_at=clock_timestamp()
      where provider='ASAAS' and provider_event_id=target_event_id;
...
revoke all on function private.process_asaas_monthly_recurring_payment_webhook(text,text,text,text,text,numeric,date,text,jsonb) from public, anon, authenticated;
revoke all on function public.process_asaas_monthly_recurring_payment_webhook(text,text,text,text,text,numeric,date,text,jsonb) from public, anon, authenticated;
grant execute on function public.process_asaas_monthly_recurring_payment_webhook(text,text,text,text,text,numeric,date,text,jsonb) to service_role;
