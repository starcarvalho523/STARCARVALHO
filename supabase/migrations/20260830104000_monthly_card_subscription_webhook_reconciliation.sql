...
begin
  ...
  with candidates as (
    ...
    where p.payment_subject_type='MONTHLY_BILLING_PERIOD'
      ...
      and t.provider_customer_id=target_provider_customer_id
      ...
      and (target_external_reference is null or t.external_reference=target_external_reference)
  )
  select count(*) into candidate_count from candidates;

  if candidate_count=0 and target_external_reference is not null then
    with candidates as (
      select p.id
      from public.payments p
      join private.payment_provider_transactions t on t.payment_id=p.id
  ...
  next_due_date := least(next_month_end,next_month_start+(extract(day from target_period.due_date)::int-1));
