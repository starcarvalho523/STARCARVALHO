...
alter table private.monthly_pix_cleanup_requests enable row level security;
revoke all on private.monthly_pix_cleanup_requests from public, anon, authenticated;

create or replace function private.monthly_pix_production_cleanup_environment_ready()
returns boolean
language sql
security definer
...
as $$
  ...
  and exists (
    select 1
     ...
     where s.name = 'ASAAS_PRODUCTION_API_KEY'
  );
$$;

create or replace function private.enqueue_expired_monthly_pix_production_cleanup_requests()
returns jsonb
language plpgsql
security definer
...
declare
  ...
  enqueued integer := 0;
begin
  if not private.monthly_pix_production_cleanup_environment_ready() then
    return jsonb_build_object('skipped', true, 'reason', 'PRODUCTION_ENVIRONMENT_NOT_READY');
  end if;

  select ds.decrypted_secret
    into api_key
    from vault.decrypted_secrets ds
   where ds.name = 'ASAAS_PRODUCTION_API_KEY'
   limit 1;

  if api_key is null or length(api_key) < 10 then
    return jsonb_build_object('skipped', true, 'reason', 'ASAAS_PRODUCTION_API_KEY_MISSING');
  end if;
  ...
  for row_item in
    select p.id as payment_id,
     ...
     where p.status = 'PENDING'
       ...
       and p.provider_environment = 'PRODUCTION'
  ...
  loop
    req_id := net.http_delete(
      url := 'https://api.asaas.com/v3/payments/' || pg_catalog.encode(convert_to(row_item.provider_payment_id,'UTF8'),'escape'),
      headers := jsonb_build_object(
        'accept','application/json',
        'access_token',api_key,
        'user-agent','StarCarvalhos-Production/monthly-pix-cleanup'
      ),
...
create or replace function private.finalize_monthly_pix_production_cleanup_requests()
returns jsonb
language plpgsql
security definer
...
create or replace function private.run_monthly_pix_production_background_cleanup()
returns jsonb
language plpgsql
security definer
...
begin
  ...
  finalized := private.finalize_monthly_pix_production_cleanup_requests();
  enqueued := private.enqueue_expired_monthly_pix_production_cleanup_requests();
  return jsonb_build_object('finalize', finalized, 'enqueue', enqueued);
end;
$$;

revoke all on function private.monthly_pix_production_cleanup_environment_ready() from public,anon,authenticated;
revoke all on function private.enqueue_expired_monthly_pix_production_cleanup_requests() from public,anon,authenticated;
revoke all on function private.finalize_monthly_pix_production_cleanup_requests() from public,anon,authenticated;
revoke all on function private.run_monthly_pix_production_background_cleanup() from public,anon,authenticated;

-- Install the production job only when the database is explicitly configured for
-- Asaas PRODUCTION. The function itself remains fail-closed until the Vault secret exists.
do $$
begin
  if exists (
    select 1
    ...
    perform cron.unschedule(jobid)
      from cron.job
     where jobname='monthly-pix-expiry-cleanup-production';

    perform cron.schedule(
      'monthly-pix-expiry-cleanup-production',
      '* * * * *',
      'select private.run_monthly_pix_production_background_cleanup();'
    );
  end if;
end;
$$;
