...
const CAPABILITIES: Record<EmployeeRole, Capability[]> = {
  ...
  operator: [], finance: ["finance:view"], auditor: ["audit:view"],
};
...
export async function getAccess() {
  ...
  const [{ data: profile }, { data: assignments }, { data: customer }] = await Promise.all([
    ...
    supabase.from("user_unit_roles").select("role,unit_id").eq("user_id", user.id),
    supabase.from("customer_profiles").select("full_name,is_active").eq("user_id", user.id).maybeSingle(),
  ]);
...
export async function requireCapability(capability: Capability) {
  ...
  if (!access.roles.some((role) => CAPABILITIES[role]?.includes(capability))) redirect("/ceo?erro=sem-permissao");
  return access;
}
