...
export class PaymentService{
  ...
  private async createPixFor(subject:PaymentSubject,userClient:SupabaseClient){
    ...
    if(context.providerPaymentId){
      ...
      try{reconciled=await provider.findPaymentByExternalReference(context.externalReference)}catch(error){
        ...
        await this.markReconciliationPending(context.transactionId,error);logPaymentError("asaas.reconcile",error);throw error;
      }
      if(reconciled){validateReconciledCharge(reconciled,context);charge=reconciled}
      else{
        if(context.state!=="CREATING"){const error=new Error("ASAAS_RECONCILIATION_NOT_FOUND");await this.markReconciliationPending(context.transactionId,error);logPaymentError("asaas.reconcile",error);throw error}
        try{
        ...
        }catch(error){await this.markReconciliationPending(context.transactionId,error);logPaymentError("asaas.create",error);throw error}
      }
    ...
    }catch(error){await this.markQrPending(context.transactionId,error);logPaymentError("asaas.pixQrCode",error);throw error}
  }
  ...
  private async createCreditCheckoutFor(subject:PaymentSubject,userClient:SupabaseClient,origin:string){
    ...
    try{
      ...
      const checkout=await provider.createCreditCardCheckout({
        ...
        description:monthly?"Pagamento de mensalidade":"Pagamento de estadia",
  ...
  async getMonthlyCreditCheckout(billingPeriodId:string,userClient:SupabaseClient){
    const{error:expireError}=await userClient.rpc("expire_monthly_credit_checkout_if_stale",{target_billing_period:billingPeriodId});if(expireError)throw new Error(expireError.message);
    const{data,error}=await userClient.rpc("get_monthly_provider_payment",{billing_period_id:billingPeriodId,payment_method:"CREDIT_CARD"});if(error)throw new Error(error.message);return data?publicCheckout(data as Reservation):null;
  }
  ...
  async reprocessStoredCheckoutEvents(sessionId:string,eventIds:string[],userClient:SupabaseClient){
    ...
    const{data,error}=await this.admin.rpc("get_checkout_payment_events_for_reprocessing",{session_id:sessionId,event_ids:eventIds});if(error)throw rpcError("get_checkout_payment_events_for_reprocessing",error.message);
    const events=(Array.isArray(data)?data:[]) as StoredCheckoutEvent[];const results=[];
    for(const event of events){const result=await this.processWebhook({id:event.id,type:event.type,paymentId:event.paymentId,paymentStatus:event.paymentStatus,amount:Number(event.amount),billingType:event.billingType,externalReference:event.externalReference,checkoutId:null,subscriptionId:null});results.push({type:event.type,result})}
    return results;
  }
  ...
  async processWebhook(event:ProviderWebhookEvent){
    ...
    if(event.billingType==="CREDIT_CARD"&&(event.type==="PAYMENT_CREATED"||event.type==="PAYMENT_CONFIRMED"))return this.processCheckoutPaymentWebhook(event,sanitized);
  ...
  private async processCheckoutPaymentWebhook(event:ProviderWebhookEvent,sanitized:Record<string,unknown>){
    if(event.amount===null){const{error:reviewError}=await this.admin.rpc("mark_checkout_payment_event_review",{event_id:event.id,event_type:event.type,provider_payment_id:event.paymentId,provider_status:event.paymentStatus,sanitized_payload:sanitized,reason_code:"CHECKOUT_PAYMENT_INVALID_AMOUNT"});if(reviewError)throw rpcError("mark_checkout_payment_event_review",reviewError.message);return{result:"review"}}
    const{data,error}=await this.admin.rpc("get_credit_checkout_reconciliation_candidates",{reported_amount:event.amount});if(error)throw rpcError("get_credit_checkout_reconciliation_candidates",error.message);
    const candidates=selectCheckoutCandidates((Array.isArray(data)?data:[]) as CheckoutCandidate[],event.amount,event.checkoutId);const provider=this.provider??getPaymentProvider();const matches=[];let requiresReview=false;
    for(const candidate of candidates){try{const resolved=await provider.resolveCheckoutPayment(candidate.checkoutId,candidate.externalReference,event.paymentId,Number(candidate.amount));if(resolved.amount===Number(candidate.amount)&&resolved.billingType==="CREDIT_CARD")matches.push({candidate,resolved})}catch(cause){const disposition=checkoutResolutionDisposition(cause);if(disposition==="NO_MATCH")continue;if(disposition==="REVIEW")requiresReview=true;logPaymentError("asaas.checkout.reconcile",cause)}}
    if(matches.length!==1){const review=requiresReview||matches.length>1;const{error:unknownError}=await this.admin.rpc("mark_checkout_payment_event_review",{event_id:event.id,event_type:event.type,provider_payment_id:event.paymentId,provider_status:event.paymentStatus,sanitized_payload:sanitized,reason_code:review?"CHECKOUT_PAYMENT_AMBIGUOUS":"CHECKOUT_PAYMENT_UNKNOWN"});if(unknownError)throw rpcError("mark_checkout_payment_event_review",unknownError.message);return{result:review?"review":"unknown"}}
    const match=matches[0];
    const{data:result,error:processError}=await this.admin.rpc("process_asaas_checkout_payment_webhook",{event_id:event.id,event_type:event.type,provider_payment_id:event.paymentId,provider_checkout_id:match.resolved.providerCheckoutId,provider_status:event.paymentStatus,reported_amount:event.amount,billing_type:event.billingType,external_reference:match.resolved.externalReference,sanitized_payload:sanitized});if(processError)throw rpcError("process_asaas_checkout_payment_webhook",processError.message);
    if(event.type==="PAYMENT_CONFIRMED"&&event.subscriptionId)await this.syncMonthlyCardRecurring(event,provider);
    return result;
  }
  ...
  private async syncMonthlyCardRecurring(event:ProviderWebhookEvent,provider:PaymentProvider){
    if(!event.subscriptionId)return;
    const{error:bindError}=await this.admin.rpc("bind_monthly_card_recurring_subscription",{target_provider_payment_id:event.paymentId,target_provider_subscription_id:event.subscriptionId});if(bindError)throw rpcError("bind_monthly_card_recurring_subscription",bindError.message);
  ...
  private async resolveProviderCustomer(subject:PaymentSubject,provider:PaymentProvider){
    const{data,error}=await this.admin.rpc("get_payment_customer_context",{subject_type:subject.type,subject_id:subject.id,target_provider:provider.name,target_environment:provider.environment});if(error)throw rpcError("get_payment_customer_context",error.message);
    const customer=data as PaymentCustomerContext;if(customer.provider_customer_id)return customer.provider_customer_id;
    const document=String(customer.billing_document??"").replace(/\D/g,"");if(!/^(?:\d{11}|\d{14})$/.test(document))throw new Error("CUSTOMER_BILLING_DOCUMENT_REQUIRED");
    let providerCustomer;try{providerCustomer=await provider.findCustomerByExternalReference(customer.external_reference);if(!providerCustomer)providerCustomer=await provider.createCustomer({name:customer.full_name,cpfCnpj:document,email:customer.email,externalReference:customer.external_reference})}catch(error){logPaymentError("asaas.customer.resolve",error);throw error}
