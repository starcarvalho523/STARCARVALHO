...
type CustomerContext={provider_customer_id:string|null};
type IgnoredProviderEvent={id:number;provider_event_id:string;provider_payment_id:string|null;sanitized_payload:unknown};

export async function reconcileIgnoredMonthlyRenewalCardSetupRpc(subscriptionId:string,userClient:SupabaseClient){
  ...
  for(const rawEvent of (events??[]) as IgnoredProviderEvent[]){
    ...
    try{
      ...
      if(snapshot.providerCustomerId!==customer.provider_customer_id||Number(snapshot.amount)!==Number(period.amount)||snapshot.dueDate!==expectedNextBillingDate)continue;
  ...
  const{error:bindingError}=await admin.from("monthly_recurring_provider_bindings").upsert({subscription_id:subscriptionId,provider:"ASAAS",method:"CREDIT_CARD",provider_customer_id:customer.provider_customer_id,provider_subscription_id:match.providerSubscriptionId,authorization_status:"ACTIVE",initial_billing_period_id:period.id,last_provider_event_id:match.event.provider_event_id,last_provider_event_at:now,updated_at:now},{onConflict:"subscription_id,method"});
  if(bindingError)throw new Error(`RENEWAL_SETUP_RECONCILIATION_BINDING_${bindingError.message}`);

  const{error:subscriptionError}=await admin.from("monthly_subscriptions").update({auto_renew:true,preferred_payment_method:"CREDIT_CARD",renewal_provider:"ASAAS",next_billing_date:expectedNextBillingDate,cancel_at_period_end:false,updated_at:now}).eq("id",subscriptionId);
  ...
  if(markError)throw new Error(`RENEWAL_SETUP_RECONCILIATION_EVENT_${markError.message}`);
  if(marked!==true)throw new Error("RENEWAL_SETUP_RECONCILIATION_EVENT_NOT_MARKED");
