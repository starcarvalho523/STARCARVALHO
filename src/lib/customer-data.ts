...
export type CustomerMonthlyPeriod={id:string;reference_year:number;reference_month:number;due_date:string;amount:number;status:string;monthly_subscriptions:{plan_name:string;unit_id:string;parking_units:{name:string}|null}|null;payments:CustomerPayment[]};
...
export const getCustomerData = cache(async () => {
  ...
  const{data:monthlyRows,error:monthlyError}=await supabase.from("monthly_billing_periods").select("id,reference_year,reference_month,due_date,amount,status,monthly_subscriptions!inner(plan_name,unit_id,parking_units(name)),payments(id,amount,method,status,paid_at,created_at)").order("reference_year",{ascending:false}).order("reference_month",{ascending:false}).limit(24);
