import "server-only";
import { getCeoAnalytics, normalizeCeoFilters, type CeoFilters, type CeoPayment, type CeoPeriod, type CeoSession } from "@/lib/ceo-analytics";
import { isOperationalFinancialPayment } from "@/lib/financial-environment";

export { normalizeCeoFilters };

export async function getCeoOperationalAnalytics(filters: CeoFilters) {
  const data = await getCeoAnalytics(filters);
  ...
  const unitSummaries = data.unitSummaries.map((unit) => ({
    ...unit,
    revenue: paid.filter((payment) => payment.unit_id === unit.id).reduce((sum, payment) => sum + Number(payment.amount), 0),
  }));
...
function makeBuckets(period: CeoPeriod, since: Date, timezone: string, paid: CeoPayment[], sessions: CeoSession[], capacity: number) {
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    if (period === "today") start.setUTCHours(start.getUTCHours() + index);
    else start.setUTCDate(start.getUTCDate() + index);
    const end = new Date(start);
    if (period === "today") end.setUTCHours(end.getUTCHours() + 1);
    ...
    const label = period === "today"
      ? new Intl.DateTimeFormat("pt-BR", { timeZone: timezone, hour: "2-digit" }).format(start)
      : new Intl.DateTimeFormat("pt-BR", { timeZone: timezone, day: "2-digit", month: "2-digit" }).format(start);
    return {
      label,
      revenue: bucketPayments.reduce((sum, payment) => sum + Number(payment.amount), 0),
      payments: bucketPayments.length,
      entries,
      exits,
      occupancy: capacity ? Math.max(0, entries - exits) / capacity * 100 : 0,
    };
  });
}
