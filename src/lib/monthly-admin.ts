...
export async function getMonthlyAccess() {
  ...
  const manageableUnitIds = [...new Set(access.assignments.filter((item) => item.role === "owner" || item.role === "manager").map((item) => item.unit_id as string))];
...
export async function getScopedCustomers(unitIds: string[]) {
  ...
  const [{ data: sessions }, { data: subscriptions }] = await Promise.all([
    ...
    admin.from("monthly_subscriptions").select("customer_id").in("unit_id", unitIds).not("customer_id", "is", null),
  ]);
  const vehicleIds = [...new Set((sessions ?? []).map((row) => row.vehicle_id as string))];
  const { data: vehicles } = vehicleIds.length ? await admin.from("vehicles").select("id,customer_id,normalized_plate,vehicle_type").in("id", vehicleIds).not("customer_id", "is", null) : { data: [] };
  const customerIds = [...new Set([...(subscriptions ?? []).map((row) => row.customer_id as string), ...(vehicles ?? []).map((row) => row.customer_id as string)])];
  const { data: customers } = customerIds.length ? await admin.from("customer_profiles").select("user_id,full_name,is_active").in("user_id", customerIds).eq("is_active", true).order("full_name") : { data: [] };
  return (customers ?? []).map((customer) => ({ ...customer, vehicles: (vehicles ?? []).filter((vehicle) => vehicle.customer_id === customer.user_id) }));
}
