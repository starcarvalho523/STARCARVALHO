...
export function MonthlyPaymentActions({billingPeriodId,allowCash=false,pendingMethod}:{billingPeriodId:string;allowCash?:boolean;pendingMethod?:string|null}){
 ...
 return <div className="mt-3 grid gap-2 sm:grid-cols-3">{allowCash?<button type="button" disabled={loading} onClick={()=>void cash()} className="flex h-16 items-center justify-center gap-2 rounded-xl border border-emerald-200 font-bold text-emerald-700 disabled:opacity-50">{loading?<LoaderCircle className="size-5 animate-spin"/>:<Banknote className="size-5"/>}Dinheiro</button>:null}<PixPaymentPanel billingPeriodId={billingPeriodId}/><CreditCheckoutPanel billingPeriodId={billingPeriodId}/>{error?<p role="alert" className="text-sm font-semibold text-red-600 sm:col-span-3">{error}</p>:null}</div>
}
