...
export function MonthlyPaymentActions({billingPeriodId,allowCash=false,pendingMethod,pixEnabled=true,creditEnabled=true}:{billingPeriodId:string;allowCash?:boolean;pendingMethod?:string|null;pixEnabled?:boolean;creditEnabled?:boolean}){
 ...
 return <div className="mt-3 grid gap-2 sm:grid-cols-3">{allowCash?<button type="button" disabled={loading} onClick={()=>void cash()} className="flex h-16 items-center justify-center gap-2 rounded-xl border border-emerald-200 font-bold text-emerald-700 disabled:opacity-50">{loading?<LoaderCircle className="size-5 animate-spin"/>:<Banknote className="size-5"/>}Dinheiro</button>:null}{pixEnabled?<PixPaymentPanel billingPeriodId={billingPeriodId}/>:null}{creditEnabled?<CreditCheckoutPanel billingPeriodId={billingPeriodId}/>:null}{!allowCash&&!pixEnabled&&!creditEnabled?<p className="rounded-xl bg-slate-100 p-3 text-sm text-slate-600 sm:col-span-3">Pagamento online indisponível nesta unidade.</p>:null}{error?<p role="alert" className="text-sm font-semibold text-red-600 sm:col-span-3">{error}</p>:null}</div>
}
