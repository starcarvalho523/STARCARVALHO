...
export function PixPaymentPanel({ sessionId, billingPeriodId, resumeExisting=false, onPaid }: { sessionId?: string; billingPeriodId?: string; resumeExisting?: boolean; onPaid?: () => void }) {
  ...
  if (!charge) {
    return <div>
      <button type="button" onClick={resumeExisting?()=>void refreshCharge():createCharge} disabled={loading||refreshing} className="flex h-16 w-full items-center justify-center gap-2 rounded-xl border border-emerald-200 font-bold text-emerald-700 hover:bg-emerald-50 disabled:opacity-50">
