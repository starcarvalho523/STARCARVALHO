...
function VehicleDetail({session,timezone}:{session:ActiveSession;timezone:string}){
  ...
  return <div><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h2 className="truncate text-2xl font-extrabold tracking-tight text-slate-950">{session.plate}</h2><p className="mt-1 flex items-center gap-2 text-xs text-slate-500"><CarFront className="size-3.5 shrink-0"/><span className="truncate">{formatVehicleType(session.vehicle_type)} • {session.tariff_name}</span></p></div><OperationBadge {...sessionParkingStatus(session.status,session.entry_mode,session.financial_obligation)}/></div>
