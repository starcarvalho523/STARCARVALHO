  ...
  return <div className="space-y-4">
    ...
    <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_390px]">
        ...
        <div className="border-b p-3 sm:p-4">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
            <label className="flex h-11 min-w-0 flex-1 items-center gap-2 rounded-xl border border-slate-200 px-3.5 transition focus-within:border-blue-400 focus-within:ring-4 focus-within:ring-blue-50">
              ...
              <input aria-label="Buscar veículo por placa" type="search" value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="Buscar por placa" className="min-w-0 flex-1 bg-transparent text-sm uppercase outline-none placeholder:normal-case"/>
            </label>
            <div className="flex flex-wrap gap-1 rounded-xl border border-slate-200 bg-slate-50/70 p-1">
              {filters.map(filter=><button key={filter.value} type="button" onClick={()=>setStatus(filter.value)} className={`rounded-lg px-3 py-2 text-xs font-semibold transition ${status===filter.value?"bg-white text-blue-700 shadow-sm ring-1 ring-blue-200":"text-slate-600 hover:bg-white"}`}>{filter.label}</button>)}
            </div>
          </div>
        </div>
      ...
      <aside className="rounded-2xl border bg-white p-4 shadow-sm xl:sticky xl:top-4 xl:self-start">
        {selected?<VehicleDetail session={selected} timezone={timezone}/>:<div className="grid min-h-72 place-items-center text-center text-sm text-slate-500"><div><CarFront className="mx-auto mb-3 size-8 text-slate-300"/><p>Selecione um veículo para ver os detalhes.</p></div></div>}
      </aside>
    </div>
  </div>;
}
...
function VehicleDetail({session,timezone}:{session:ActiveSession;timezone:string}){
  ...
  return <div><div className="flex items-start justify-between gap-3"><div><h2 className="text-2xl font-extrabold tracking-tight text-slate-950">{session.plate}</h2><p className="mt-1 flex items-center gap-2 text-xs text-slate-500"><CarFront className="size-3.5"/>{formatVehicleType(session.vehicle_type)} • {session.tariff_name}</p></div><OperationBadge {...sessionParkingStatus(session.status,session.entry_mode,session.financial_obligation)}/></div>
...
function Detail({label,value,accent=false,wide=false}:{label:string;value:string;accent?:boolean;wide?:boolean}){return <div className={wide?"col-span-2":""}><p className="text-[10px] text-slate-500">{label}</p><p className={`mt-1 text-xs font-semibold ${accent?"text-emerald-600":"text-slate-950"}`}>{value}</p></div>}
function DetailRow({label,value,strong=false}:{label:string;value:string;strong?:boolean}){return <div className="flex items-start justify-between gap-4"><span className="text-xs text-slate-500">{label}</span><b className={`text-right text-xs ${strong?"text-blue-600":"text-slate-800"}`}>{value}</b></div>}
