...
export function AnalyticsChart({ title, rows, field, empty }: { title: string; rows: ChartRow[]; field: "revenue" | "occupancy"; empty: string }) {
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
    ...
    {max === 0 ? <div className="grid min-h-24 place-items-center px-4 py-4 text-center"><div><BarChart3 className="mx-auto size-7 text-slate-200"/><p className="mt-2.5 text-sm font-medium text-slate-500">{empty}</p><p className="mt-1 text-xs text-slate-400">{emptyHint}</p></div></div> :
...
export function AlertList({ alerts, limit }: { alerts: CeoAlert[]; limit?: number }) {
  ...
  return <div className="space-y-2 px-4 pb-4">{rows.map((alert) => {
    ...
    return <article key={alert.id} className={`rounded-xl border p-3.5 ${critical?"border-red-100 bg-red-50/70":"border-amber-100 bg-amber-50/70"}`}>
      <div className="flex items-start gap-3"><span className={`mt-0.5 h-fit rounded-full px-2.5 py-1 text-[10px] font-bold ${critical ? "bg-red-100 text-red-700" : alert.severity === "Atenção" ? "bg-amber-100 text-amber-700" : "bg-blue-100 text-blue-700"}`}>{alert.severity}</span><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><p className="min-w-0 flex-1 font-bold leading-5 text-slate-950">{alert.title}</p><Link href={alert.href} className="inline-flex shrink-0 items-center gap-1 whitespace-nowrap text-xs font-bold text-blue-600 hover:text-blue-700">{action}<ArrowUpRight className="size-3.5"/></Link></div><p className="mt-0.5 text-xs font-medium text-slate-500">{alert.unitName}</p><p className="mt-1 text-xs leading-5 text-slate-500">{alert.description}</p></div></div>
    </article>;
  })}</div>;
}
...
export function InsightGrid({ entries, averageMinutes, occupancy, ticket }: { entries: number; averageMinutes: number; occupancy: number; ticket: number }) {
  ...
  return <div className="grid h-full auto-rows-fr gap-2 sm:grid-cols-2">{data.map(({label,value,icon:Icon,tone}) => <div key={label} className="flex min-h-[94px] min-w-0 items-start gap-3 rounded-xl border border-slate-200 bg-white p-3"><span className={`grid size-9 shrink-0 place-items-center rounded-xl ${tone}`}><Icon className="size-4"/></span><div className="min-w-0"><p className="text-[11px] leading-4 text-slate-500">{label}</p><p className="mt-1 break-words text-sm font-bold leading-5 text-slate-950">{value}</p></div></div>)}</div>;
}
