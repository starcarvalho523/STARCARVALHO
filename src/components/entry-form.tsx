...
export function EntryForm({carEnabled,motorcycleEnabled}:{carEnabled:boolean;motorcycleEnabled:boolean}){
  ...
  return <div className="space-y-3">
    <form action={action} className="grid gap-3 lg:grid-cols-[1fr_180px_280px]">
      ...
      <label className="sr-only" htmlFor="vehicleType">Tipo do veículo</label><select id="vehicleType" name="vehicleType" value={vehicleType} onChange={event=>setVehicleType(event.target.value as VehicleType)} className="h-16 rounded-xl border bg-white px-4 font-semibold"><option value="CAR" disabled={!carEnabled}>Carro{!carEnabled?" — sem tarifa":""}</option><option value="MOTORCYCLE" disabled={!motorcycleEnabled}>Moto{!motorcycleEnabled?" — sem tarifa":""}</option></select>
      <button disabled={pending||!valid||!selectedTypeEnabled} className="flex h-16 items-center justify-center gap-3 rounded-xl bg-blue-600 font-bold text-white shadow-lg hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50">{pending?<LoaderCircle className="size-5 animate-spin"/>:<LogIn className="size-5"/>}{pending?"Verificando...":"Registrar entrada"}</button>
    </form>
    {noTariffAvailable?<p className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">Nenhuma tarifa ativa disponível para entrada. Configure ao menos uma tarifa de carro ou moto antes de registrar veículos.</p>:null}
