...
export function EntryForm({carEnabled,motorcycleEnabled}:{carEnabled:boolean;motorcycleEnabled:boolean}) {
  ...
  </form>{(!carEnabled||!motorcycleEnabled)&&<p className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">Tarifa ausente para: {[!carEnabled&&"carro",!motorcycleEnabled&&"moto"].filter(Boolean).join(" e ")}. Entradas desse tipo estão bloqueadas.</p>}{(state.error||state.success)&&<p role="status" className={`flex items-center gap-2 rounded-xl px-4 py-3 text-sm font-semibold ${state.error?"bg-red-50 text-red-700":"bg-emerald-50 text-emerald-700"}`}><CheckCircle2 className="size-5"/>{state.error??state.success}</p>}</div>;
}
