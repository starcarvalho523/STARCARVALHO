...
export function MonthlyRenewalControls({
  ...
  const act=async(action:"ENABLE"|"DISABLE"|"CANCEL_AT_PERIOD_END")=>{
    ...
    try{
      ...
      if(action==="ENABLE")setLocalAutoRenew(true);
      if(action==="DISABLE")setLocalAutoRenew(false);
      if(action==="CANCEL_AT_PERIOD_END")setLocalCancelAtPeriodEnd(true);
      router.refresh();
    }catch(cause){setError(cause instanceof Error?cause.message:"Não foi possível atualizar a renovação.")}
    finally{setLoading(null)}
  };

  if(localCancelAtPeriodEnd){
  ...
  return <section className="mt-4 rounded-2xl border border-blue-200 bg-blue-50/60 p-4 sm:p-5" aria-busy={Boolean(loading)}>
    <div className="flex flex-wrap items-start justify-between gap-4">
      ...
      <div className="rounded-xl border bg-white px-4 py-3 text-sm"><div className="flex items-center gap-2 font-semibold"><CalendarClock className="size-4"/>Próxima cobrança</div><p className="mt-1 text-slate-600">{localAutoRenew?date(nextBillingDate):"Desativada"}</p></div>
    </div>

    <div className="mt-4 grid gap-2 sm:grid-cols-2">
      {localAutoRenew?<button type="button" disabled={Boolean(loading)} onClick={()=>void act("DISABLE")} className="flex h-11 items-center justify-center gap-2 rounded-xl border bg-white font-bold text-slate-700 disabled:opacity-50">{loading==="DISABLE"?<LoaderCircle className="size-4 animate-spin"/>:<RefreshCw className="size-4"/>}{loading==="DISABLE"?"Atualizando no Asaas...":"Desativar renovação automática"}</button>:<button type="button" disabled={Boolean(loading)} onClick={()=>void act("ENABLE")} className="flex h-11 items-center justify-center gap-2 rounded-xl bg-blue-600 font-bold text-white disabled:opacity-50">{loading==="ENABLE"?<LoaderCircle className="size-4 animate-spin"/>:<RefreshCw className="size-4"/>}{loading==="ENABLE"?"Atualizando no Asaas...":"Ativar renovação automática"}</button>}
      <button type="button" disabled={Boolean(loading)} onClick={()=>void act("CANCEL_AT_PERIOD_END")} className="flex h-11 items-center justify-center gap-2 rounded-xl border border-red-200 bg-white font-bold text-red-700 disabled:opacity-50">{loading==="CANCEL_AT_PERIOD_END"?<LoaderCircle className="size-4 animate-spin"/>:<XCircle className="size-4"/>}{loading==="CANCEL_AT_PERIOD_END"?"Atualizando...":"Cancelar assinatura ao fim do período"}</button>
    </div>
    {loading?<p className="mt-3 text-sm font-semibold text-blue-700" role="status">Confirmando a alteração com o provedor. A tela será sincronizada em segundo plano.</p>:null}
