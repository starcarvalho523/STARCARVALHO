...
import { formatDateTime, formatDuration, formatMoney, formatParkingStatus, formatPaymentMethod, formatVehicleType } from "@/lib/operator-format";
...
export default async function Page({ searchParams }: { searchParams: Promise<{ period?: string; unit?: string; session?: string }> }) {
  ...
  return <DashboardShell nav={ceoNav} active="Relatórios" role="CEO"><div className="mx-auto max-w-[1500px] space-y-5">
    ...
    <div className="grid gap-4 lg:grid-cols-3">
      ...
      <Report title="Financeiro" rows={[["Receita",formatMoney(data.metrics.revenue)],["Ticket médio",data.metrics.ticket?formatMoney(data.metrics.ticket):"—"],["Dinheiro",formatMoney(data.methods.CASH.amount)],["Cartão",formatMoney(data.methods.CARD.amount)],["PIX","Não disponível"],["Pagamentos",data.metrics.payments]]}/>
    ...
    {selected ? <section className="rounded-2xl border border-blue-100 bg-white p-5"><div className="flex items-center justify-between"><div><p className="text-xs text-slate-500">Detalhe da sessão</p><h2 className="text-xl font-bold">{selected.plate_snapshot}</h2></div><Link className="text-sm text-blue-600" href={`/ceo/relatorios?period=${data.filters.period}&unit=${data.filters.unitId}`}>Fechar</Link></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><Cell label="Entrada" value={formatDateTime(selected.entered_at)}/><Cell label="Saída" value={selected.exited_at?formatDateTime(selected.exited_at):"—"}/><Cell label="Veículo" value={formatVehicleType(selected.vehicle_type)}/><Cell label="Status" value={formatParkingStatus(selected.status)}/><Cell label="Valor" value={formatMoney(selected.final_amount??selected.calculated_amount)}/></div></section> : null}
    <Table title="Sessões" empty={!data.sessions.length?"Nenhuma sessão encontrada neste período.":undefined}><thead><tr>{["Placa","Entrada","Saída","Status","Valor","Ação"].map((label)=><th key={label} className="px-4 py-3">{label}</th>)}</tr></thead><tbody>{data.sessions.map((session)=><tr key={session.id} className="border-t"><td className="px-4 py-3 font-bold">{session.plate_snapshot}</td><td>{formatDateTime(session.entered_at)}</td><td>{session.exited_at?formatDateTime(session.exited_at):"—"}</td><td>{formatParkingStatus(session.status)}</td><td>{formatMoney(session.final_amount??session.calculated_amount)}</td><td><Link className="font-semibold text-blue-600" href={sessionHref(session.id)}>Ver sessão</Link></td></tr>)}</tbody></Table>
    <Table title="Pagamentos detalhados" empty={!data.payments.length?"Nenhum pagamento encontrado neste período.":undefined}><thead><tr>{["Data","Placa","Entrada","Método","Status","Valor","Ação"].map((label)=><th key={label} className="px-4 py-3">{label}</th>)}</tr></thead><tbody>{data.payments.map((payment)=><tr key={payment.id} className="border-t"><td className="px-4 py-3">{formatDateTime(payment.paid_at??payment.created_at)}</td><td className="font-bold">{payment.parking_sessions?.plate_snapshot??"—"}</td><td>{payment.parking_sessions?.entered_at?formatDateTime(payment.parking_sessions.entered_at):"—"}</td><td>{formatPaymentMethod(payment.method,payment.manual_confirmation)}</td><td>{payment.status==="PAID"?"Pago":payment.status}</td><td>{formatMoney(payment.amount)}</td><td>{payment.parking_sessions?<Link className="font-semibold text-blue-600" href={sessionHref(payment.parking_session_id)}>Ver sessão</Link>:"—"}</td></tr>)}</tbody></Table>
  </div></DashboardShell>;
}
