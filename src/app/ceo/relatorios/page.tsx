      ...
      <div className="mx-auto max-w-[1500px] space-y-4">
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <DataSection
            title="Sessões"
            icon={Clock3}
            className={data.sessions.length ? "lg:col-span-2" : ""}
          ...
          <DataSection
            ...
            icon={CreditCard}
            tone="green"
            className={data.payments.length ? "lg:col-span-2" : ""}
      ...
      <div className="flex items-center gap-3 border-b border-slate-100 pb-3">
        ...
        <div>
          ...
          <div className="mt-0.5 flex flex-wrap items-baseline gap-x-2">
            <p className={`text-2xl font-extrabold leading-none ${palette.value}`}>{primaryValue}</p>
      ...
      <dl className={centerRows ? "flex flex-1 flex-col justify-center gap-2 py-3" : "mt-2"}>
        {rows.map(([label, value], index) => {
          ...
          return (
            <div
              key={label}
              className={`flex items-center justify-between gap-4 border-b border-slate-100 px-1 py-1.5 text-sm last:border-0 ${
                highlighted ? `mt-1 rounded-xl border-0 px-3 py-2 ${palette.highlight}` : ""
              }`}
            >
  ...
  return (
    <section className="rounded-[22px] border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-start gap-3 border-b border-slate-100 pb-3">
        <span className={`grid size-10 shrink-0 place-items-center rounded-2xl ${palette}`}>
          <Icon className="size-5" />
        </span>
        <div>
          <h2 className="font-extrabold text-slate-950">{title}</h2>
...
function DataSection({
  ...
  return (
    <section
      className={`overflow-hidden rounded-[22px] border border-slate-200 bg-white shadow-sm ${className}`}
    >
      <div className="flex items-center gap-3 border-b border-slate-100 px-5 py-3.5">
        <span className={`grid size-9 place-items-center rounded-xl ${palette}`}>
      ...
      {empty ? (
        <div className="flex min-h-[118px] items-center justify-center gap-4 px-6 py-5 text-slate-500">
