...
export default async function Page({searchParams}:{searchParams:Promise<{period?:string;unit?:string}>}){
  ...
  return <DashboardShell nav={ceoNav} active="Painel do CEO" role="CEO">
    <div className="mx-auto max-w-[1500px] space-y-4 sm:space-y-5">
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-[0.95fr_1.3fr_0.95fr_0.9fr_0.9fr_0.95fr]">
      ...
      <section className="grid items-stretch gap-4 xl:grid-cols-[1fr_1.05fr_1.1fr]">
        <PaymentMethods methods={data.methods}/>

        <section className="flex min-h-full flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center justify-between gap-3 px-5 py-4"><div><h2 className="font-bold text-slate-950">Alertas prioritários</h2><p className="mt-0.5 text-xs text-slate-400">{data.alerts.length?`${data.alerts.length} alerta${data.alerts.length===1?"":"s"} ativo${data.alerts.length===1?"":"s"}`:"Operação sem alertas prioritários"}</p></div><Link href="/ceo/alertas" className="shrink-0 text-xs font-bold text-blue-600 hover:text-blue-700">Ver todos</Link></div>
          <div className="flex-1"><AlertList alerts={data.alerts} limit={3}/></div>
        </section>

        <section className="flex min-h-full flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="flex-1"><InsightGrid entries={data.metrics.entries} averageMinutes={data.metrics.averageMinutes} occupancy={data.metrics.occupancy} ticket={data.metrics.ticket}/></div>
        </section>
      </section>
