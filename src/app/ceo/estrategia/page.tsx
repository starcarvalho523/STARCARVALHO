  ...
  return <DashboardShell nav={ceoNav} active="Estratégia" role="CEO">
    <main className="mx-auto max-w-[1500px] space-y-4 pb-8">
      ...
      <section className="grid gap-3 xl:grid-cols-[1.1fr_.9fr]">
        ...
        {canManage ? <GlobalForm action={saveZone} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><SectionTitle icon={Layers3} title="Criar zona"/><div className="mt-3 grid gap-2.5 sm:grid-cols-2"><UnitSelect units={manageableUnits}/><Label text="Código"><input name="code" className={field} required placeholder="A" /></Label><Label text="Nome"><input name="name" className={field} required placeholder="Rotatividade frente" /></Label><Label text="Capacidade"><input name="capacity" type="number" min="1" className={field} required /></Label><Label text="Tipo"><select name="zoneType" className={field} defaultValue="FLEX"><option value="ROTATION">Rotatividade</option><option value="FLEX">Flexível</option><option value="MONTHLY">Mensalistas</option><option value="RESERVED">Reservada</option></select></Label><Label text="Prioridade"><input name="priority" type="number" min="1" max="100" defaultValue="50" className={field} required /></Label></div><input type="hidden" name="active" value="true" /><button className="mt-3 h-9 w-full rounded-lg bg-blue-600 text-sm font-bold text-white hover:bg-blue-700">Salvar zona</button></GlobalForm> : null}
      </section>
      ...
      <section className="grid gap-3 xl:grid-cols-2">
        <Card title="Perfil econômico da unidade" subtitle="Base para custo por vaga, receita por m² e futuro break-even.">
          ...
          {canManage && <GlobalForm action={saveUnitCommercialProfile} className="mt-3 grid gap-2.5 sm:grid-cols-3"><UnitSelect units={manageableUnits}/><Label text="Área útil (m²)"><input name="areaSqm" type="number" min="1" step="0.01" className={field}/></Label><Label text="Custo fixo mensal"><input name="monthlyFixedCost" type="number" min="0" step="0.01" className={field}/></Label><button className="h-9 rounded-lg bg-slate-950 text-sm font-bold text-white sm:col-span-3">Atualizar perfil econômico</button></GlobalForm>}
        </Card>
      ...
      <section className="grid gap-3 xl:grid-cols-2">
        <Card title="Aquisição de clientes" subtitle="Primeira origem conhecida para medir CAC sem virar um CRM.">
          ...
          {canManage && customers.length>0 && <GlobalForm action={saveAcquisitionSource} className="mt-3 grid gap-2.5 sm:grid-cols-2"><UnitSelect units={manageableUnits}/><Label text="Cliente"><select name="customerId" className={field} required>{customers.map((c)=><option key={c.id} value={c.id}>{c.name}{c.email ? ` · ${c.email}` : ""}</option>)}</select></Label><Label text="Origem"><ChannelSelect name="source"/></Label><Label text="Campanha"><input name="campaign" className={field} placeholder="Opcional"/></Label><button className="h-9 rounded-lg bg-blue-600 text-sm font-bold text-white sm:col-span-2">Salvar origem</button></GlobalForm>}
        </Card>
