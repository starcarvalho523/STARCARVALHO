    ...
    <DashboardShell nav={ceoNav} active="Clientes" role="CEO">
      <div className="mx-auto max-w-7xl space-y-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Base administrativa derivada de vínculos reais com veículos, passagens e mensalidades das suas unidades.</p>
        </div>
        ...
        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <Kpi icon={UsersRound} label="Clientes ativos" value={active} tone="blue" />
          <Kpi icon={CarFront} label="Com veículos" value={withVehicles} tone="slate" />
          <Kpi icon={WalletCards} label="Mensalistas" value={monthly} tone="violet" />
          <Kpi icon={CircleCheckBig} label="Com sessão ativa" value={inYard} tone="emerald" />
        </section>
        ...
        <form className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:grid-cols-[minmax(0,1fr)_220px_190px_190px_auto]" action="/ceo/clientes">
          <label className="relative block">
            ...
            <input name="q" defaultValue={params.q ?? ""} placeholder="Buscar cliente, e-mail ou unidade" className="h-12 w-full rounded-xl border border-slate-200 bg-white pl-11 pr-4 text-sm outline-none focus:border-blue-400" />
          </label>
          <select name="unit" defaultValue={unitFilter} className="h-12 rounded-xl border border-slate-200 bg-white px-4 text-sm outline-none focus:border-blue-400">
            <option value="all">Todas as unidades</option>
            {units.map((unit) => <option key={unit.id} value={unit.id}>{unit.name}</option>)}
          </select>
          <select name="type" defaultValue={typeFilter} className="h-12 rounded-xl border border-slate-200 bg-white px-4 text-sm outline-none focus:border-blue-400">
            <option value="all">Todos os tipos</option>
            <option value="monthly">Mensalistas</option>
            <option value="casual">Avulsos</option>
          </select>
          <select name="status" defaultValue={statusFilter} className="h-12 rounded-xl border border-slate-200 bg-white px-4 text-sm outline-none focus:border-blue-400">
            <option value="all">Todos os status</option>
            <option value="active">Ativos</option>
            <option value="inactive">Inativos</option>
            <option value="yard">No pátio agora</option>
          </select>
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-100 px-5 py-4">
            <h2 className="font-extrabold text-slate-950">Base de clientes</h2>
            <p className="mt-1 text-sm text-slate-500">{filtered.length === 1 ? "1 cliente encontrado" : `${filtered.length} clientes encontrados`}</p>
          </div>
          <div className="overflow-x-auto">
            <table className="min-w-[1050px] w-full text-left text-sm">
              <thead className="bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-500">
                <tr><th className="px-5 py-3">Cliente</th><th className="px-5 py-3">Unidade</th><th className="px-5 py-3">Veículos</th><th className="px-5 py-3">Última passagem</th><th className="px-5 py-3">Mensalidade</th><th className="px-5 py-3">Situação</th><th className="px-5 py-3 text-right">Ação</th></tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {filtered.map((customer) => (
                  <tr key={customer.customer_id} className="align-middle">
                    ...
                    <td className="px-5 py-4 text-slate-600">{customer.units[0]?.name ?? "—"}{customer.units.length > 1 ? ` +${customer.units.length - 1}` : ""}</td>
                    <td className="px-5 py-4"><span className="font-bold text-slate-900">{customer.vehicle_count}</span><span className="text-slate-500"> {customer.vehicle_count === 1 ? "veículo" : "veículos"}</span></td>
                    <td className="px-5 py-4 text-slate-600">{customer.last_visit_at ? formatDateTime(customer.last_visit_at) : "Sem passagem"}</td>
                    <td className="px-5 py-4"><span className={monthlyBadge(customer.monthly_status)}>{monthlyLabel(customer.monthly_status)}</span>{customer.monthly_plan ? <p className="mt-1 max-w-[180px] truncate text-xs text-slate-500">{customer.monthly_plan}</p> : null}</td>
                    <td className="px-5 py-4"><div className="flex flex-wrap gap-1.5">{customer.has_active_session ? <Badge label="No pátio" tone="blue" /> : null}{customer.eligible_for_monthly && !customer.monthly_status ? <Badge label="Elegível para mensalista" tone="emerald" /> : null}{!customer.is_active ? <Badge label="Cadastro inativo" tone="slate" /> : null}{customer.is_active && !customer.has_active_session && !(customer.eligible_for_monthly && !customer.monthly_status) ? <Badge label="Ativo" tone="emerald" /> : null}</div></td>
                    <td className="px-5 py-4 text-right"><Link href={`/ceo/clientes/${customer.customer_id}`} className="inline-flex items-center gap-1 font-bold text-blue-600 hover:text-blue-700">Abrir cliente <ArrowUpRight className="size-4" /></Link></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="rounded-2xl border border-blue-100 bg-blue-50 px-5 py-4 text-sm text-blue-800">
          <strong>Origem segura:</strong> esta base não cria clientes administrativos paralelos. Um cliente aparece quando possui passagem ou mensalidade vinculada a uma unidade que você administra.
        </div>
      </div>
    </DashboardShell>
  );
...
function Kpi({ icon: Icon, label, value, tone }: { icon: typeof UsersRound; label: string; value: number; tone: "blue" | "slate" | "violet" | "emerald" }) {
  const tones = { blue: "bg-blue-50 text-blue-600", slate: "bg-slate-100 text-slate-600", violet: "bg-violet-50 text-violet-600", emerald: "bg-emerald-50 text-emerald-600" };
