    ...
    >
      <div className="space-y-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {data.unreadNotifications ? <GlobalForm action={markAllNotificationsRead}><button className="min-h-11 rounded-xl border bg-white px-4 text-sm font-bold text-blue-600">Marcar todas como lidas</button></GlobalForm> : null}
        </div>
        ...
        <section className="max-h-[calc(100dvh-14rem)] space-y-3 overflow-y-auto overscroll-contain pb-4">
          {data.notifications.map((item) => <article key={item.id} className={`rounded-2xl border p-4 ${item.read_at ? "bg-white" : "border-blue-200 bg-blue-50"}`}><div className="flex gap-3"><Bell className="mt-1 size-5 shrink-0 text-blue-600"/><div className="min-w-0 flex-1"><div className="flex flex-wrap justify-between gap-2"><h2 className="font-bold">{item.title}</h2><time className="text-xs text-slate-500">{formatDateTime(item.created_at)}</time></div><p className="mt-1 break-words text-sm text-slate-700">{item.message}</p><div className="mt-3 flex flex-wrap gap-3">{item.internal_link?<Link href={item.internal_link} className="text-sm font-bold text-blue-600">Abrir</Link>:null}{!item.read_at?<GlobalForm action={markNotificationRead}><input type="hidden" name="notificationId" value={item.id}/><button className="text-sm font-semibold text-slate-600">Marcar como lida</button></GlobalForm>:null}</div></div></div></article>)}
