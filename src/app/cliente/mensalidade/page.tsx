            ...
            <article key={period.id} className="rounded-2xl border bg-white p-5">
              <div className="flex flex-wrap justify-between gap-3">
                <div>
                  ...
                  {subscriptionStatus==="ACTIVE"&&period.status!=="PAID"&&latestPaidCoverage?(cardAutoRenewActive?<div className="mt-3 rounded-xl bg-blue-50 px-3 py-2 text-sm text-blue-800"><b>Assinatura ativa.</b> Sua cobertura já paga vai até <b>{date(latestPaidCoverage)}</b>. A próxima cobrança será feita automaticamente no cartão em <b>{date(subscription?.next_billing_date??period.due_date)}</b>.</div>:<div className="mt-3 rounded-xl bg-blue-50 px-3 py-2 text-sm text-blue-800"><b>Assinatura ativa.</b> Sua cobertura já paga vai até <b>{date(latestPaidCoverage)}</b>. Esta é a próxima mensalidade e ainda aguarda pagamento.</div>):null}
