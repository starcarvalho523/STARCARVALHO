      ...
      <div className="space-y-5">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Competências e pagamentos vinculados exclusivamente à sua conta.
          </p>
        </div>
          ...
          return (
            ...
            >
              <div className="flex flex-wrap justify-between gap-3">
                <div>
                  <h2 className="font-bold">
                    {period.monthly_subscriptions?.plan_name ?? "Plano mensal"}
                  </h2>
                  <p className="text-sm text-slate-500">
                    {period.monthly_subscriptions?.parking_units?.name ??
                      "Star Carvalhos"}{" "}
                    · {String(period.reference_month).padStart(2, "0")}/
                    {period.reference_year}
                  </p>
                  <p className="mt-2 text-sm">
                    Vencimento:{" "}
                    {new Date(`${period.due_date}T12:00:00`).toLocaleDateString(
                      "pt-BR",
                    )}
                  </p>
                </div>
                <div className="text-right">
                  <b className="text-2xl">{formatMoney(period.amount)}</b>
              ...
              {period.status === "PENDING" && !pending ? (
                <MonthlyPaymentActions billingPeriodId={period.id} />
              ) : null}
              {paid ? (
                <div className="mt-4 rounded-xl bg-emerald-50 p-3 text-sm text-emerald-800">
                  {formatPaymentMethod(paid.method)} ·{" "}
                  {formatPaymentStatus(paid.status)} ·{" "}
                  {formatDateTime(paid.paid_at ?? paid.created_at)}
                </div>
              ) : pending ? (
                <div className="mt-4 rounded-xl bg-amber-50 p-3 text-sm text-amber-800">
                  Cobrança em processamento. Não inicie outro método.
                </div>
              ) : null}
            </article>
          );
        })}
        {!data.monthlyPeriods.length&&!plans.length ? (
          <p className="rounded-2xl border bg-white p-8 text-center text-sm text-slate-500">
            Nenhuma competência vinculada à sua conta.
          </p>
        ) : null}
      </div>
    </CustomerShell>
