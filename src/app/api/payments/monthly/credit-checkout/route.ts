...
export async function POST(request:Request){
  try{const body=await request.json();const billingPeriodId=typeof body?.billingPeriodId==="string"?body.billingPeriodId:"";if(!billingPeriodId)return Response.json({error:"BILLING_PERIOD_ID_REQUIRED"},{status:400});const origin=new URL(request.url).origin;const checkout=await new PaymentService().createMonthlyCreditCheckout(billingPeriodId,await createClient(),origin);return Response.json({checkout},{headers:{"cache-control":"no-store"}})}catch(error){return failure(error)}
}
