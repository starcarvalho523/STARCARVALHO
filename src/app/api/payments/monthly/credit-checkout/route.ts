...
type CheckoutSnapshot={state?:unknown;hostedPaymentUrl?:unknown};
...
export async function POST(request:Request){
  try{
    ...
    // A retry/double click must reuse an already-created checkout instead of
    // starting the same monthly payment flow again and turning a success into 503.
    const existing=await service.getMonthlyCreditCheckout(billingPeriodId,client) as CheckoutSnapshot|null;
    if(existing&&typeof existing.hostedPaymentUrl==="string"&&existing.hostedPaymentUrl&&!["CREATE_FAILED","EXPIRED","CANCELLED"].includes(String(existing.state??""))){
      return Response.json({checkout:existing},{headers:{"cache-control":"no-store"}});
    }

    await prepareMonthlyPaymentAttempt(billingPeriodId,"CREDIT_CARD",client);
    const checkout=await service.createMonthlyCreditCheckout(billingPeriodId,client,origin);
