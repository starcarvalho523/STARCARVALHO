...
function failure(error:unknown){const message=error instanceof Error?error.message:"PAYMENT_REQUEST_FAILED";const forbidden=/FORBIDDEN|SESSION_NOT_FOUND/.test(message);const unavailable=/NOT_CONFIGURED|ASAAS_HTTP/.test(message);return Response.json({error:forbidden?"PAYMENT_NOT_AVAILABLE":unavailable?"PAYMENTS_SANDBOX_UNAVAILABLE":"PAYMENT_REQUEST_FAILED"},{status:forbidden?404:unavailable?503:400})}
