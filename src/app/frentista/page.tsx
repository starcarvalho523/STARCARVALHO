...
export default async function OperatorPage(){const data=await getOperatorDashboard();const shift=data.open_shift;if(!shift)redirect("/frentista/caixa?welcome=1");return <DashboardShell nav={operatorNav} active="Painel" role="Frentista" aside={<div className="rounded-2xl border bg-white p-4 shadow-sm"><p className="text-sm font-bold">Caixa do turno</p><><p className="mt-3 text-xs text-slate-500">Dinheiro esperado</p><p className="text-xl font-bold text-emerald-600">{formatMoney(Number(shift.opening_amount)+Number(shift.cash_total))}</p><p className="mt-2 text-xs text-slate-500">{shift.payment_count} pagamentos</p></><Link href="/frentista/caixa" className="mt-4 block rounded-lg border py-2 text-center text-xs font-semibold text-blue-600">Ver caixa</Link></div>}><div className="mx-auto max-w-[1400px] space-y-5">
  ...
  <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label="Veículos no pátio" value={String(data.vehicles_in_yard)} icon={CarFront}/><MetricCard label="Vagas disponíveis" value={String(data.available_spaces)} icon={Gauge} tone="green"/><MetricCard label="Entradas hoje" value={String(data.entries_today)} icon={LogIn} tone="violet"/><MetricCard label="Saídas hoje" value={String(data.exits_today)} icon={LogOut} tone="orange"/></div>
  ...
  <section className="overflow-hidden rounded-2xl border bg-white shadow-sm"><div className="flex items-center justify-between border-b px-5 py-4"><h2 className="font-bold">Veículos no pátio</h2><Link href="/frentista/veiculos" className="text-sm font-semibold text-blue-600">Ver todos</Link></div><OperatorSessionTable sessions={data.active_sessions} timezone={data.unit.timezone} limit={8}/></section>
</div></DashboardShell>}
