...
function ClosedShiftGreeting({shift}:{shift:ClosedShiftSummary|null}){const received=shift?.expected_cash_amount==null?null:Math.max(0,shift.expected_cash_amount-shift.opening_amount);const difference=shift?.difference_amount??null;return <section className="relative overflow-hidden rounded-[30px] border border-blue-200/90 bg-[radial-gradient(circle_at_50%_0%,#ffffff_0%,#f6f9ff_45%,#eef5ff_100%)] px-4 py-6 text-center shadow-[0_24px_65px_rgba(30,64,175,0.12)] sm:rounded-[34px] sm:px-10 sm:py-7"><ClosedConfetti/><div className="relative mx-auto max-w-[650px]"><ClosedMedal/><h2 className="mt-4 text-[29px] font-black leading-tight tracking-[-0.025em] text-slate-950 sm:mt-5 sm:text-[36px]">Caixa fechado com sucesso! 🎉</h2><p className="mt-1.5 text-[20px] font-extrabold text-blue-600 sm:text-[24px]">Mais um turno concluído!</p><p className="mx-auto mt-3 max-w-[570px] text-[15px] leading-6 text-slate-600 sm:text-base sm:leading-6">Tudo conferido, tudo certo. Seu esforço e dedicação<br className="hidden sm:block"/> fazem a nossa operação seguir em frente.</p><p className="mt-3 text-[16px] font-extrabold text-blue-600 sm:text-lg">💙 Você é essencial para o nosso sucesso! 💙</p>{shift?<div className="mx-auto mt-5 max-w-[655px] overflow-hidden rounded-[21px] border border-blue-200 bg-white/80 text-left shadow-[0_12px_30px_rgba(37,99,235,0.07)] backdrop-blur-sm"><FinanceRow icon={Wallet} label="Saldo inicial" value={formatMoney(shift.opening_amount)}/><FinanceRow icon={CircleDollarSign} label="Total recebido" value={received==null?"—":formatMoney(received)}/><FinanceRow icon={Calculator} label="Total contado" value={shift.declared_cash_amount==null?"—":formatMoney(shift.declared_cash_amount)}/><FinanceRow icon={Scale} label="Diferença" value={difference==null?"—":formatMoney(difference)} emphasis tone={difference===0?"text-blue-600":difference!=null?"text-amber-600":"text-slate-700"}/></div>:null}<Link href="/frentista" className="mx-auto mt-4 flex h-14 w-full max-w-[655px] items-center justify-center gap-3 rounded-[17px] border border-blue-500/40 bg-[linear-gradient(135deg,#2563eb_0%,#3b82f6_54%,#4f46e5_100%)] px-6 text-base font-extrabold text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.3),0_14px_26px_rgba(37,99,235,0.28)] transition hover:-translate-y-0.5 hover:brightness-105 sm:text-lg"><LayoutDashboard className="size-5"/>Ir para o painel</Link></div></section>}
function ClosedMedal(){return <div className="relative mx-auto h-[108px] w-[108px] sm:h-[115px] sm:w-[115px]"><span className="absolute left-1/2 top-[57px] h-9 w-8 -translate-x-[34px] rotate-[16deg] rounded-b-sm bg-[linear-gradient(180deg,#60a5fa,#2563eb)] shadow-[0_7px_9px_rgba(37,99,235,.22)]"/><span className="absolute left-1/2 top-[57px] h-9 w-8 -translate-x-[2px] -rotate-[16deg] rounded-b-sm bg-[linear-gradient(180deg,#3b82f6,#1d4ed8)] shadow-[0_7px_9px_rgba(37,99,235,.22)]"/><span className="absolute inset-[12px] rounded-full border-[6px] border-blue-100 bg-blue-200/45 shadow-[0_11px_24px_rgba(37,99,235,.16)]"/><span className="absolute inset-[20px] grid place-items-center rounded-full border-[4px] border-blue-300/70 bg-[radial-gradient(circle_at_32%_24%,#7dd3fc_0%,#3b82f6_43%,#1d4ed8_100%)] text-white shadow-[inset_0_-6px_10px_rgba(30,64,175,.28),inset_0_4px_7px_rgba(255,255,255,.35)]"><span className="absolute left-[18%] top-[14%] size-3 rounded-full bg-white/20 blur-[1px]"/><CheckCircle2 strokeWidth={3.1} className="relative size-10 drop-shadow-[0_2px_2px_rgba(30,64,175,.25)] sm:size-11"/></span></div>}
function ClosedConfetti(){const pieces=["left-[13%] top-9 h-2 w-1 rotate-[33deg] bg-blue-600","left-[20%] top-14 h-1.5 w-4 -rotate-[10deg] bg-blue-400","left-[28%] top-7 h-2 w-1 rotate-[45deg] bg-indigo-500","left-[37%] top-12 h-1.5 w-3 -rotate-[35deg] bg-blue-200","left-[44%] top-6 h-1 w-2 rotate-[17deg] bg-blue-300","right-[13%] top-9 h-2 w-1 -rotate-[33deg] bg-blue-600","right-[20%] top-14 h-1.5 w-4 rotate-[10deg] bg-blue-400","right-[28%] top-7 h-2 w-1 -rotate-[45deg] bg-indigo-500","right-[37%] top-12 h-1.5 w-3 rotate-[35deg] bg-blue-200","right-[44%] top-6 h-1 w-2 -rotate-[17deg] bg-blue-300"];return <div className="pointer-events-none absolute inset-x-0 top-2 h-20" aria-hidden="true">{pieces.map((piece,index)=><span key={index} className={`absolute rounded-sm ${piece}`}/>)}</div>}
function FinanceRow({icon:Icon,label,value,emphasis=false,tone="text-slate-950"}:{icon:typeof Wallet;label:string;value:string;emphasis?:boolean;tone?:string}){return <div className="flex items-center justify-between gap-4 border-b border-blue-100 px-5 py-3 last:border-b-0 sm:px-6"><span className="flex items-center gap-3 text-[15px] text-slate-600 sm:text-base"><Icon className="size-5 shrink-0 text-blue-600"/>{label}</span><b className={`text-[15px] sm:text-base ${emphasis?tone:"text-slate-950"}`}>{value}</b></div>}
