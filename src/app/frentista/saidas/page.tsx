...
import { formatSessionFinancialStatus, sessionParkingStatus } from "@/lib/operator-format";
...
export default async function ExitsPage({ searchParams }: { searchParams: Promise<{ session?: string }> }) {
  ...
  return <DashboardShell nav={operatorNav} active="Saídas" role="Frentista"><div className="mx-auto max-w-6xl space-y-5">
    ...
    <div className="grid gap-4 lg:grid-cols-[.85fr_1.15fr]">
      ...
      {selected ? <section className="rounded-2xl border bg-white p-6 shadow-sm"><div className="flex justify-between gap-4"><div><p className="text-sm text-slate-500">Placa</p><h2 className="text-3xl font-bold">{selected.plate}</h2></div><OperationBadge {...sessionParkingStatus(selected.status,selected.entry_mode,selected.financial_obligation)} /></div><dl className="my-6 grid grid-cols-1 gap-4 border-y py-5 text-sm sm:grid-cols-2"><div><dt className="text-slate-500">Entrada</dt><dd className="font-semibold">{formatDateTime(selected.entered_at, data.unit.timezone)}</dd></div><div><dt className="text-slate-500">Permanência</dt><dd className="font-semibold">{formatDuration(selected.duration_minutes)}</dd></div><div><dt className="text-slate-500">Tarifa</dt><dd className="font-semibold">{selected.tariff_name}</dd></div><div><dt className="text-slate-500">Valor a cobrar</dt><dd className="text-xl font-bold text-emerald-600">{formatMoney(selected.amount)}</dd></div></dl>
        {selected.financial_obligation === "WAIVED_BY_MONTHLY_COVERAGE" ? <div className="mb-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-900"><p className="font-bold">Estadia coberta por mensalidade</p><p className="mt-1 text-sm">{formatSessionFinancialStatus(selected.entry_mode,selected.financial_obligation)}</p><p className="mt-3 text-sm font-semibold">Valor a cobrar: {formatMoney(0)}</p><p className="mt-1 text-xs text-emerald-700">Valor avulso teórico: {formatMoney(selected.theoretical_amount)}</p></div> : null}
        {selected.status === "OPEN" ? <StartExitForm sessionId={selected.id} monthly={selected.financial_obligation === "WAIVED_BY_MONTHLY_COVERAGE"} /> : null}
        {selected.status === "PAYMENT_PENDING" && selected.financial_obligation !== "WAIVED_BY_MONTHLY_COVERAGE" ? <div className="space-y-4"><div><p className="text-sm font-semibold">Registrar recebimento</p><div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-3">{cashEnabled?<PaymentForm sessionId={selected.id} method="CASH"/>:null}{pixEnabled?<PixPaymentPanel sessionId={selected.id}/>:null}{creditEnabled?<CreditCheckoutPanel sessionId={selected.id}/>:null}</div></div></div> : null}
