...
export async function registerEntry(_:OperatorActionState,formData:FormData):Promise<OperatorActionState>{
  ...
  if(error){
    if(error.message.includes("PARKING_FULL")){
      await supabase.rpc("record_parking_demand_loss",{target_unit:unitId,target_reason:"FULL",target_vehicle_type:vehicleType,target_notes:`Entrada recusada automaticamente · ${plate}`});
      refresh();
      return{error:humanError(error.message),relatedMessage:"A recusa por lotação foi registrada para o CEO."};
    }
  ...
  const modeLabel=result?.entry_mode==="BUSINESS"?" como empresarial":result?.entry_mode&&result.entry_mode!=="CASUAL"?" como mensalista":"";
  return{success:`Entrada de ${plate} registrada${modeLabel}.`,sessionId:String(result?.session_id??""),relatedMessage:lastStay?.exited_at?`Última estadia finalizada em ${new Intl.DateTimeFormat("pt-BR",{timeZone:unit?.timezone??"America/Bahia",dateStyle:"short",timeStyle:"short"}).format(new Date(lastStay.exited_at))}.`:undefined,relatedHref:lastStay?`/frentista/historico?q=${plate}&status=all&period=30&session=${lastStay.id}`:undefined};
}
